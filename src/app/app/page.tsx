...
export default async function Dashboard() {
  ...
  const studentCount = db.select({ n: count() }).from(students).where(and(eq(students.academyId, ctx.academy.id), isNull(students.archivedAt))).get()?.n ?? 0;
  const classCount = db.select({ n: count() }).from(classes).where(eq(classes.academyId, ctx.academy.id)).get()?.n ?? 0;
  const lessonCount = db.select({ n: count() }).from(lessons).where(and(eq(lessons.academyId, ctx.academy.id), gte(lessons.lessonDate, today))).get()?.n ?? 0;
  const draftCount = db.select({ n: count() }).from(generatedDrafts).where(and(eq(generatedDrafts.academyId, ctx.academy.id), eq(generatedDrafts.status, "draft"))).get()?.n ?? 0;
